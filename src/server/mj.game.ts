...
export class Game {
  ...
  print() {
    ...
    // console.log("Wall East:     ", this.walls[0].tiles.map((id) => MjCore.tileFromId(id).name).join(", "));
    // console.log("Wall South:    ", this.walls[1].tiles.map((id) => MjCore.tileFromId(id).name).join(", "));
    // console.log("Wall West:     ", this.walls[2].tiles.map((id) => MjCore.tileFromId(id).name).join(", "));
    // console.log("Wall North:    ", this.walls[3].tiles.map((id) => MjCore.tileFromId(id).name).join(", "));
    if (this.players[0]) {
      console.log("Player East:   ", this.players[0]?.handTiles.map((id) => MjCore.tileFromId(id).name).join(", "));
      console.log("       picked: ", this.players[0]?.picked ? MjCore.tileFromId(this.players[0]?.picked).name : "");
    }
    if (this.players[1]) {
      console.log("Player South:  ", this.players[1]?.handTiles.map((id) => MjCore.tileFromId(id).name).join(", "));
      console.log("       picked: ", this.players[1]?.picked ? MjCore.tileFromId(this.players[1]?.picked).name : "");
    }
    if (this.players[2]) {
      console.log("Player West:   ", this.players[2]?.handTiles.map((id) => MjCore.tileFromId(id).name).join(", "));
      console.log("       picked: ", this.players[2]?.picked ? MjCore.tileFromId(this.players[2]?.picked).name : "");
    }
    if (this.players[3]) {
      console.log("Player North:  ", this.players[3]?.handTiles.map((id) => MjCore.tileFromId(id).name).join(", "));
      console.log("       picked: ", this.players[3]?.picked ? MjCore.tileFromId(this.players[3]?.picked).name : "");
    }
    console.log("Dealer:        ", this.dealer?.position);
    console.log("Current:       ", this.current?.position);
    console.log("Pick Position: ", this.pickPosition);
    console.log("Pick Index:    ", this.pickIndex);
  }
  ...
  extractAllPaires(tiles: TileId[]) {
    ...
    while (i < tiles.length - 1) {
      if (MjCore.tileFromId(tiles[i]).name !== MjCore.tileFromId(tiles[i + 1]).name) {
        continue;
      }
      pairs.push([i, i + 1]);
      ...
      while (i < tiles.length - 1 && MjCore.tileFromId(first).name === MjCore.tileFromId(tiles[i + 1]).name) {
        i++;
      }
    }
    return tiles;
